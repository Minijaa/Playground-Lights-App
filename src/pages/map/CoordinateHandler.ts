...
var latOfOrigin = null;
var scale = null;
var falseNorthing = null;
var falseEasting = null;
export class CoordinateHandler {
  ...
  geodeticToGrid(lat, long) {
    ...
    var e2 = flattening * (2.0 - flattening),
      ...
      eta_prim = this.mathAtanh(Math.cos(phi_star) * Math.sin(delta_lambda)),
      x = scale * a_roof * (xi_prim +
        beta1 * Math.sin(2.0 * xi_prim) * this.mathCosh(2.0 * eta_prim) +
        beta2 * Math.sin(4.0 * xi_prim) * this.mathCosh(4.0 * eta_prim) +
        beta3 * Math.sin(6.0 * xi_prim) * this.mathCosh(6.0 * eta_prim) +
        beta4 * Math.sin(8.0 * xi_prim) * this.mathCosh(8.0 * eta_prim)) +
        falseNorthing,
      y = scale * a_roof * (eta_prim +
        beta1 * Math.cos(2.0 * xi_prim) * this.mathSinh(2.0 * eta_prim) +
        beta2 * Math.cos(4.0 * xi_prim) * this.mathSinh(4.0 * eta_prim) +
        beta3 * Math.cos(6.0 * xi_prim) * this.mathSinh(6.0 * eta_prim) +
        beta4 * Math.cos(8.0 * xi_prim) * this.mathSinh(8.0 * eta_prim)) +
        falseEasting;
